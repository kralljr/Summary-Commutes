...
```{r}
snowptab <- select(weather1, date_local, ID, contains("snowbin"), contains("prcpbin"), -windsnow) %>%
  pivot_longer(-c("ID", "date_local")) %>%
  group_by(name) %>%
...
dat <- select(dat, id, age : numchildren)
...
ords <- c("Asian", "White only", "Other/Did not specify",
          ...
          "Yes","No",  "None",  "1+")
demo <- select(dat1, id, Race, Ethnicity, Education, GMUstudent, Employed, Children) %>%
  pivot_longer(-id) %>%
  na.omit() %>%
  group_by(name) %>%
  count(value) %>%
