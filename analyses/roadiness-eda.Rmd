...
```{r}
dplyr::select(sum1, ID, lmean, mean,sd) %>% pivot_longer(-ID) %>%
  group_by(name) %>% summarize(med = median(value), mean = mean(value), sd = sd(value)) %>%
  kable(digits =2)


dplyr::select(sum1, ID, min, max) %>%  summarize(min = min(min), max = max(max))
```
...
ns <- dplyr::select(rcomm1, ID, medl, n) %>% unique()
...
rcomm1 <- mutate(rcomm, id2 = fct_reorder(id2, timemin)) %>%
  arrange(id2) %>%
  group_by(id2) %>%
  mutate(mean1 = mean(lPM, na.rm = T), sd1 = sd(lPM, na.rm = T),
         lPM2 = (lPM - mean1) / sd1,
         rtype2 = factor(rtype, levels = c("High/SecHigh", "Other", "LocalConn", "Local"),
                         labels = c("Highway", "Ramp/Tunnel", "LocalConn", "Local"))) %>%
  ungroup()
...
pmtile <- ggplot(rcomm1, aes(x = timemin, y = id2, fill = lPM2)) +
  geom_tile() + ylab("Trip") + xlab("Minutes since trip start") +
  scale_fill_gradient2(name = "Change in PM2.5 (SD)", low = "#18c732", high = "#FF3333",
                       mid = "white") +
  theme_bw() +
  theme(text = element_text(size=12), axis.text.y = element_blank(),
...
dplyr::select(rcomm, ID, date_local, group, rtypeMode) %>% unique() %>%
...
s11 <- rcomm %>% 
  dplyr::select(ID, id2, srness1loc, srness500loc, srness1hw, srness500hw) %>%
...
s11 %>%
  ...
  kable(digits = 2)
ggplot(s11, aes(x = value)) + geom_histogram(bins = 20) +
  facet_wrap(~name)



# by ID
g1 <- s11 %>% group_by(name, ID) %>% summarize(mean = mean(value), sd = sd(value), min = min(value), max = max(value))  %>% ungroup()
g1 %>% kable(digits = 2)
...
```{r}
s1 <- summarize(rcomm, mean = mean(mph), sd = sd(mph), min = min(mph), max= max(mph),
                med = median(mph), IQR = IQR(mph))
s1 %>%
  kable(digits = 2)

# ggplot(rcomm, aes(x = mph)) + geom_boxplot()
# by ID
g1 <- group_by(rcomm, ID) %>% summarize(mean = mean(mph), sd = sd(mph), min = min(mph), max= max(mph),
                med = median(mph), IQR = IQR(mph))  %>% ungroup()
g1 %>% kable(digits = 2)

speed1 <- mutate(rcomm1, speed = case_when(mph < 1 ~ "stop", 
                                           mph >= 1 & mph < 10 ~ "slow",
                                           mph >= 10 & mph < 30 ~ "local",
                                           mph >= 30 & mph < 50 ~ "mod",
                                           mph >= 50 ~ "hwy"))

speedtile <- ggplot(speed1, aes(x = timemin, y = id2, fill = speed)) +
  geom_tile() + ylab("Trip") + xlab("Minutes since trip start") +
  scale_fill_colorblind(name = "Speed (mph)") +
  #scale_fill_gradient(name = "Speed (mph)", low = "#FFFF33", high = "#FF3333") +
  theme_bw() +
  theme(text = element_text(size=12), axis.text.y = element_blank(),
        axis.ticks.y = element_blank(),
        legend.position  = c(.75, .25))
...
dplyr::select(rcomm1, id2, ID, timemin) %>%
  group_by(ID, id2) %>%
  summarize(maxt = max(timemin)) %>%
  unique() %>%
  arrange(desc(maxt))
```
...
weather1 <- dplyr::select(rcomm, date_local, ID, awnd : RH, snowbin: prcpbinL1m, tavg) %>%
  unique() %>%
...
```{r}
snowptab <- dplyr::select(weather1, date_local, ID, contains("snowbin"), contains("prcpbin"), -windsnow) %>%
  pivot_longer(-c("ID", "date_local")) %>%
  group_by(name) %>%
  count(value) %>% mutate(perc = round(100* n/sum(n), 1),
                          NPerc = paste0(n, " (", perc, ")"),
                          value = ifelse(value== 0, "None", "Some"),
                          name = case_when(name == "snowbin"~ "Snow",
                                           name == "prcpbin" ~ "Precipitation")) %>%
  dplyr::select(name, value, NPerc) %>%
  dplyr::filter(name %in% c("Snow", "Precipitation")) %>%
...
```{r}
weather1b <- dplyr::select(rcomm, date_local, ID, group, awnd : snowL1m,snowbin: prcpbinL1m, tavg) %>%
  unique() %>%
  dplyr::select(-contains("wdf"))
...
contweat <- group_by(weatherb, name) %>% summarize(mean = mean(value), sd = sd(value), 
                min= min(value), max = max(value))
contweat%>%
  kable(digits = 2)


contweat1 <- dplyr::filter(contweat, name %in% c("awnd", "tmax", "tmin", "tavg")) %>%
  mutate(name = case_when(name == "awnd"~ "Wind speed (m/s)",
                          name == "tmax" ~ "Maximum temp (C)",
                          name == "tmin" ~ "Minimum temp (C)",
                          name == "tavg" ~ "Average temp (C)"),
         mean = round(mean, 1), sd = round(sd, 1),
         MeanSD = paste0(mean, " (", sd, ")")) %>%
  dplyr::select(name, MeanSD) %>%
...
```{r}
dplyr::select(weather1b, date_local, ID, group, contains("snowbin"), contains("prcpbin"), -windsnow) %>%
...
```{r}
winddir <- dplyr::select(rcomm, ID, date_local, cat2sm, cat5sm, wdf2, wdf5) %>% unique()
...
- Wind dir: 5
```{r}
winddirt <- count(winddir, cat5sm) %>% mutate(perc = round(100 * n / sum(n), 1), NPerc =paste0(n, " (", perc, ")")) %>%
  dplyr::select(cat5sm, NPerc) %>%
  rename(Value = cat5sm) %>%
  mutate(Variable = "Wind direction")

catweath <- bind_rows(snowptab, winddirt)
...
```{r}
winddir1 <- rename(winddir, dir = wdf5)
...
```{r}
dates <- dplyr::select(rcomm, date_local, ID) %>% unique()
if(nrow(dates) != 45) {
...
```{r, eval = T}
part <- dplyr::select(rcomm, ID) %>% unique() %>%
  mutate(id = tolower(ID))
load("~/Documents/gestdc-data/gestdc-merged.RData")

dat <- dplyr::select(dat, id, age : numchildren)

dat1 <- left_join(part, dat) %>%
  mutate(Children = ifelse(numchildren == 0,
                              "None", "1+"),
         GMUstudent = ifelse(occupation == "George Mason Student", "Yes", "No"),
         Race = ifelse(race == "White", "White only",
                       ifelse(race %in% c("Asian", "White,Asian"), "Asian", "Other/Did not specify"))) %>%
      #   Education = ifelse(Education)) %>%
  rename(Ethnicity = ethnicity, Education = education, Employed = fulltime)

summarize(dat1, mean(age), sd(age), min(age), max(age))

ords <- c("Asian", "White only", "Other/Did not specify",
          ...
          "Yes","No",  "None",  "1+")
demo <- dplyr::select(dat1, id, Race, Ethnicity, Education, GMUstudent, Employed, Children) %>%
  pivot_longer(-id) %>%
  na.omit() %>%
  group_by(name) %>%
  count(value) %>%
  mutate(perc= 100 *n/25, perc = round(perc, 1),
         nperc = paste0(n, " (", perc, ")")) %>%
  dplyr::select(name, value, nperc) %>%
  rename(Variable = name, Value = value, NPerc = nperc) %>%
  mutate(Value = factor(Value, levels = ords)) %>%
  arrange(Value)
kable(demo)
# education
# (5 + 5 + 2 + 7) / 25
...
save(winddir1, catweath,contweat1,  demo, file = here("results/supp-plots.RData"))
```
