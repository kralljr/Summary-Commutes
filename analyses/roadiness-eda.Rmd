...
summarize(dat, mean(age, na.rm = T), sd(age,na.rm = T), min(age,na.rm = T), max(age,na.rm = T))

summarize(dat1, mean(age), sd(age), min(age), max(age))

select(dat1, id, marital, race, ethnicity, education, occupation, fulltime, numchildren) %>%
  pivot_longer(-id) %>%
  na.omit() %>%
  group_by(name) %>%
  count(value) %>%
  mutate(perc= 100 *n/25, perc = round(perc, 1)) %>% View()
