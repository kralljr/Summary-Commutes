...
```{r, eval = F}
# check GMU1040
gmu1040 <- filter(gpslatlon$dat, ID == "GMU1040")
colnames(gmu1040)[2] <- "date"
#2019-02-13
gmu1040 <- filter(gmu1040, substr(date, 1, 5) == "13 Fe") %>%
  mutate(hour = as.numeric(substr(date, 18, 19)),
                           minute = as.numeric(substr(date, 21, 22)))


as_datetime(as.POSIXct("2019-02-13 00:10:10", tz = "GMT"), tz = "America/New_York") 

as_datetime(as.POSIXct("2019-02-13 08:48:00", tz = "America/New_York"), tz = "GMT") 

gmu1040 %>%
  filter((hour == 13 & minute > 55) | (hour == 14 & minute < 2)) %>% View(  )
...
```{r}
comparepm <- ungroup(comparepm)
save(pairedpoint, comparepm, file = here("data/commutecompare-res.RData"))
```
