...
sc <- group_by(coms3, IDcommute) %>% mutate(md = median(diffPM))  %>%
  dplyr::select(-c(propcs, diffPM)) %>%
  unique() 
...
coms3 %>% dplyr::select(IDcommute, diffPM, daily) %>%
  group_by(IDcommute) %>%
  mutate(adiffPM = abs(diffPM - daily)) %>%
  summarize(med = median(adiffPM), IQR = IQR(adiffPM),
            q1 = quantile(adiffPM, probs = 0.25), 
                        q3 = quantile(adiffPM, probs = 0.75)) %>%
