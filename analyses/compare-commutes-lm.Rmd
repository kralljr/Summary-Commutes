...
sc <- group_by(coms3, IDcommute) %>% mutate(adiffPM = diffPM - daily, md = median(diffPM))  %>%
...
## Check for in-vehicle RH


```{r}
# in-vehicel RH
load("data/inrh.RData")
cinrh <- left_join(coms2, inrh) %>%
  dplyr::select(-c(rdatetime, PM)) %>%
  group_by(ID, commute, min2, propcs) %>%
  summarize(rtiRH = mean(rtiRH, na.rm = T)) %>%
  ungroup() %>%
  pivot_wider(names_from = min2, values_from = rtiRH) %>%
  arrange(ID, commute, propcs) %>%
  group_by(ID, commute) %>%
  mutate(lagsecond = dplyr::lag(second), leadsecond= dplyr::lead(second),
         second = ifelse(is.na(second), (lagsecond + leadsecond) / 2, second)) %>%
  dplyr::select(-c(lagsecond, leadsecond)) %>%
  na.omit() %>%
  mutate(diffinrh = second - first) %>%
  dplyr::select( -c(first, second)) %>%
  full_join(comspaired) %>%
  mutate(IDcommute = paste0(ID, commute))
cinrh <- cinrh %>%
  dplyr::filter( !(ID == "GMU1001" & commute== "evening" & propcs < 6.26 & propcs >6.24))

cinrh1 <- group_by(cinrh, IDcommute) %>% 
  mutate(md = median(diffPM), value = median(diffinrh)) %>%
  dplyr::select(-c(propcs, diffPM, diffinrh, first, second)) %>%
  unique() 
...
coms3b2
ids <- substr(coms3b2$IDcommute, 1, 7)
unique(ids) %>% length()
data.frame(ids, duplicated(ids))
