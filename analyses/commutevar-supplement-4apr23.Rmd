...
## Table S`r k`. Correlations of road features with ambient PM$_{2.5}$ and daily meteorology. Variables include: Road density: road density for highways and local roads at both 1 km and 500 m  (standard deviations); Speed (MPH); DailyPM: daily ambient PM$_{2.5}$ ($\mu$g/m$^3$);  HourlyDevPM: hourly deviations from daily ambient PM$_{2.5}$ ($\mu$g/m$^3$); WindSpeed (m/s); MaxTemp: maximum temperature ($^{\circ}$C); MinTemp: minimum temperature ($^{\circ}$C); and Humid: relative humidity (%)).
...
## Table S`r k`. Minimum, maximum, and quantiles (P25 = 25%ile, P50 = 50%ile, P75 = 75%ile) for road density (for highways and local roads at both 1 km and 500 m) and speed.
...
```{r}
kable(quanttab[c(2 : 5, 1),], digits = 2)
```
...
```{r}
hw500 + ggtitle("A. Highway") +
  loc500 + title("B. Local")
