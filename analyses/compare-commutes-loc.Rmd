...
```{r}
# add distance for each pair within commute
...
# by time
# comspaired1 <- mutate(coms2, dt1 = lubridate::date(rdatetime)) %>%
#             group_by(ID, commute, dt1) %>%
#             mutate(start = min(rdatetime),
#                 time = as.numeric((rdatetime - start) / 60)) %>%
#   ungroup() %>%
#   dplyr::select(-c(rdatetime, dt1, start)) %>%
#   pivot_wider(names_from = min2, values_from = PM)
# 

# by proportion of commute
comspaired <- dplyr::select(coms2, -rdatetime) %>%
  ...
  na.omit() %>%
  mutate(diffPM = second - first)

save(coms2, comspaired, file = here("data/commute-loc.RData"))
