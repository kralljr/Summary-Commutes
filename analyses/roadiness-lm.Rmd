...
```{r}
rcommU <- dplyr::select(rcomm, srness, rtype, id2, PM, daily, obsdiff, ID,date_local,group, awnd , prcp , tmax, snow, tmin, cat5sm, timemin) %>% na.omit()

iqrs <- dplyr::select(rcommU, -c(PM, ID, date_local, group,  
                                prcp, snow, rtype, id2, cat5sm)) %>%
...
lme1d <- lme(PM ~ srness + daily + obsdiff + awnd + prcp + tmax + snow + tmin + rtype + timemin + cat5sm, random=~timemin|ID/date_local/group, data=rcommU, correlation= cs1)
...
t1 <- tidy(lme1d, conf.int = T) %>%
  ...
  mutate(IQR = ifelse(is.na(IQR), 1, IQR),
         ...
         term1 = factor(term, levels = c("daily", "obsdiff", "srness", "timemin",
                          ...
                          "tmax" ,"tmin",  "prcp", "snow" , "awnd", "cat5smNW", "cat5smOther"),
         labels = c("Daily PM2.5", "Hourly PM2.5", "Roadiness", "Time since start",
                    "Local conn", "Local", "Other",
                    "Max Temp",  "Min Temp", "Precipitation","Snow", "Wind", "WindDir NW", "WindDir Other")),
         type = ifelse(term %in% c("daily", "obsdiff", "srness",   "rtypeLocalConn",  "rtypeLocal",    "rtypeOther", "timemin"), "Pollution", "Meteorology"),
         type2 = ifelse(term %in% c("prcp", "snow",  "rtypeLocalConn",  "rtypeLocal",    "rtypeOther", "cat5smNW", "cat5smOther"), "cat", "num")) 
```
