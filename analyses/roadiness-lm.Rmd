...
```{r}
rcommU <- dplyr::select(rcomm, srness, rtype, PM, daily, obsdiff, ID,date_local,group, awnd , prcp , tmax, snow, tmin) %>% na.omit()
...
iqrs <- iqrs %>%
  ...
  mutate(IQR = ifelse(name %in% c("snow", "prcp"), 1, IQR)) %>%
  rename(term = name)
...
lme1d <- lme(PM ~ srness + daily + obsdiff + awnd + prcp + tmax + snow + tmin + rtype, random=~1|ID/date_local/group, data=rcommU, correlation= cs1)
```
...
t1 <- tidy(lme1d, conf.int = T) %>%
  ...
  mutate(IQR = ifelse(is.na(IQR), 1, IQR),
         ...
         term1 = factor(term, levels = c("daily", "obsdiff", "srness", 
                            "rtypeLocalConn",  "rtypeLocal",    "rtypeOther", 
                          "tmax" ,"tmin",  "prcp", "snow" , "awnd"),
         labels = c("Daily PM2.5", "Hourly PM2.5", "Roadiness", 
                    "Local conn", "Local", "Other",
                    "Max Temp",  "Min Temp", "Precipitation","Snow", "Wind")),
         type = ifelse(term %in% c("daily", "obsdiff", "srness",   "rtypeLocalConn",  "rtypeLocal",    "rtypeOther"), "Pollution", "Meteorology"),
         type2 = ifelse(term %in% c("prcp", "snow",  "rtypeLocalConn",  "rtypeLocal",    "rtypeOther"), "cat", "num")) 
```
